...
import { SubCategoryOption } from '~helpers'
...
const ActionSheetPickerSubOptions: React.FC<ActionSheetInputProps> = ({
  control,
  name,
  rules,
  options,
    ...
    (
      ...
      <>
        <Pressable
          ...
          onPress={() => {
            if (!item.subCategoryOptions) { // Select if there are no suboptions
              onChange(item.value);
              closeActionSheet();
              setQuery('');
            }
        ...
        {item.subCategoryOptions && item.subCategoryOptions.length > 0 && (
          <Box paddingLeft="16px">
            {item.subCategoryOptions.map((subItem) => (
              <Pressable
                key={subItem.value}
                flexDirection="row"
                justifyContent="space-between"
                paddingY="8px"
                backgroundColor={subItem.value === value ? 'green.10' : null}
                onPress={() => {
                ...
                <Text
                  fontSize="14px"
                  lineHeight="18px"
                  color={subItem.value === value ? 'green.50' : 'charcoal'}>
                  {subItem.label}
                </Text>
                {subItem.value === value ? <Check /> : <Box />}
              </Pressable>
            ))}
          </Box>
        )}
  ...
  const findOption = (value: string) => {
    for (const option of options) {
      ...
      if (option.subCategoryOptions && !isEmpty(option.subCategoryOptions)) {
        // Find and return the specific subOption if it exists
        const subOption = option.subCategoryOptions.find(subOption => subOption.value === value);
        if (subOption) {
          return subOption;
        }
      }

      // If no subCategoryOption matches, check the main option's value
      if (option.value === value) {
        return option;
      }
    }
    // Return null if no matching option or subOption is found
    return null;
  };
...
export default ActionSheetPickerSubOptions
