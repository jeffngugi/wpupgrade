...
type FileScrollViewProps = {
  ...
  hasDelete?: boolean
  hasEdit?: boolean
  hasApiDelete?: boolean
  onApiDelete?: (file: any) => void
}
...
export const FileScrollView = ({
  ...
  const handleRemoveFile = (file: FileData, type, fileIdx) => {
    if (hasApiDelete) {
      onApiDelete?.(file)
      return
    }
    handleRemove(file, type, fileIdx)
  }
          ...
          {allFiles.map(({ file, type, fileIdx }: any, index: number) => {
            return (
              <Menu
                ...
                {hasDelete && (
                  <Menu.Item onPress={() =>
                    handleRemoveFile(file, type, fileIdx)
                  }>
                    <Text fontFamily={'body'} fontSize={'16px'}>
                      Delete
                    </Text>
