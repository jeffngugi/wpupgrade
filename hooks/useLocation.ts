...
export const useLocation = (options?: UseLocationProps) => {
  ...
  const requestForegroundPermission = useCallback(async () => {
    ...
    if (!isLocationEnabled)
      await enableNetworkProviderAsync().catch(err => console.log(err))
  ...
  const requestAddresses = useCallback(
    async (query: string): Promise<CompoundLocation[]> => {
  ...
  const requestLocationPermission = async () => {
    if (isIos) {
      ...
      const IosPermissionReq = await request(
...
const makeLocationKey = (location: LocationGeocodedLocation) => {
  const key = Object.entries(location).reduce(
    (result, prop) => result + propToKey(prop),
    '',
  )
  return key
}
